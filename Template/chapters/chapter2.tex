...
\end{figure}
 ...
 \section{Battery capacity calculation}
 \label{sec:batcap}
 
 6U satellite is a new step forward with an availability of payloads and different subsystems. However, use of the new payloads and subsystems require a huge amount of power which should be stored in the batteries. Batteries must be able to provide a power to the subsystem which will be enough to accomplish the mission requirements. In order to understand the required capacity of the batteries for the 6U satellite mission, it is important to do a battery capacity calculation. 
 
 The detailed code of the battery capacity calculation is described in Annex A. 
 
Despite the fact that satellite is working on the LEO orbit, high of the orbit is 600km

Due to the fact that high of the orbit is known, time of one orbital circle could be found:

 \begin{equation}\label{eq:time}
T = 2\pi\sqrt{\frac{R_{LEO}^{3}}{\mu}}
 \end{equation}
...
R_{LEO} = R_{e} + h
 \end{equation}
  ...
  With\\ $ R_{e}$ = Earth radius\\
        $     h$ = Orbit high\\
         $\mu$ - gravitational constant
 
Despite the fact that satellite fly on the LEO orbit, orbital eclipse has to be taken in to account.

\begin{equation}\label{eq:ec}
eclipse = 2sin(\frac{R_{e}}{R_{LEO}})
\end{equation}

Now it is possible to determine a light time of orbit
 \begin{equation}\label{eq:day}
 T_{day} = T - eclipse
 \end{equation}
 During the day, satellite will gain an energy from the sun\\
 
 \begin{equation}\label{eq:ec}
 charge = P \times K \times T_{day}
 \end{equation}
Where P is a power and K is the coefficient which might differ according to the type of batteries. For the  described cause coefficient is 1.3 according to the datasheet.
 
 During the flight satellite consume some amount of energy due to working payload and subsystems. Discharge of the satellite batteries can be described as: \\
 \begin{equation}\label{eq:ec}
 discharge = P \times K \times dt
 \end{equation}
 Where $dt$ is the time of discharge. According to the different modes and their timelines discharge value may vary. 
 
 For each orbit difference between charge and discharge will define the battery capacity and from battery capacity to state of charge. According to the power budget of the satellite, 6U cubesat will have 4 modes:\\ \\
 $\bullet$ Mode 1 - 161.47 A on which the satellite ...\\ 
 $\bullet$ Mode 2 - 97.59 A on which the satellite ...\\
 $\bullet$ Mode 3 - 6.94 A on which the satellite ...\\
 $\bullet$ Mode 4 - 39.37 A on which the satellite ...\\
 
Each mode involves the inclusion of different subsystems or payloads, which was calculated by a system engineer according to the datasheets of the subsystems and payloads used for the 6U satellite. 

Figure \ref{fig: PB} represents the state of charge of the 6U satellite with the actual current and actual time of modes. Plot shows state of charge of beginning of life and the end of life. Little stars on the plot illustrate the orbit intervals, where time of 1 orbit is 1h 61min. At the beginning satellite perform .... Second mode .... Third mode... Fourth mode... Important requirement of the state of charge of the 6U satellite is that its state of charge should not decrease bellow the threshold of the 60\% which is in the tolerance frame on the figure \ref{fig: PB}. The proper battery capacity should be picked up to fulfill upper mentioned requirements. As a result of the energy simulation on the figure \ref{fig: PB} conclusion can be made, that battery with capacity of 160 Wh can accomplish this requirement. 
 
 \begin{figure}[h]
 	\centering
 	\includegraphics[scale=0.7]{PB.png}
 	\caption{State of charge of 160 Wh battery }
 	\label{fig: PB}
 \end{figure}
  ...
  Due to the energy requirements based on section \ref{sec:batcap}, 6U satellite has to provide a 160 Wh of energy and  distribute it to 23 channels. To accomplish this requirements, considering the available space of the 6U satellite, EPS was devided into 3 main parts which connectrd between each other via PC104 connector:\\ \\
 ...
 Battery pack board used to provide a power storage for the EPS by using of 4 li-ion batteries with a 2 series 2 parallel configuration. According to the Appendix \ref{sec:batcap}, one of the EPS requirements is to provide capacity of 160 Wh which can be made by using 2 battery packs with LI-ion batteries samsung INR21700 with a capacity of 4900mAh.
  ...
  \begin{figure}[h]
  	\centering
  	\includegraphics[scale=0.20]{BP.png}
  ...
  The detailed review, calculations and all explanations of the Power Distribution board will be given in chapter \ref{sec:tech77}
    ...
    \begin{figure}[h]
    	\centering
    	\includegraphics[scale=0.18]{PDU.png}
    	\caption{Power Distribution Unit}
    	\label{fig: PDU}
    \end{figure}
